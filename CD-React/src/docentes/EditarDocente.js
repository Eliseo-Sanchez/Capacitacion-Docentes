...
export default function EditarDocente() {
    ...
    const cargarDocente = async () => {
        const resultado = await axios.get(`http://localhost:8080/cd-app/docentes/${id}`);
        setDocente(resultado.data);
    }
    ...
    const onSubmit = async (e) => {
        e.preventDefault();
        const urlBase = `http://localhost:8080/cd-app/docentes/${id}`;
        await axios.put(urlBase, docente);
